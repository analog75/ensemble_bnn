...
class SimpleBinActive(torch.autograd.Function):
    '''
    ...
    def backward(self, grad_output):
        ...
        grad_input[input.ge(1)]  = grad_input[input.ge(1)] * 0.01 
        grad_input[input.le(-1)] = grad_input[input.le(-1)] * 0.01 
        return grad_input
...
class BasicBlock_BiReal(nn.Module):
#{{{
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock_BiReal, self).__init__()

        #self.binary_activation = BinaryActivation()
        self.binary_activation = SimpleBinaryActivation()
    ...
    def forward(self, x):
        ...
        # with simple binary activation
        #out = SimpleBinActive.apply(x)
...
#{{{
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.binary_activation1 = SimpleBinActive.apply
        self.binary_activation2 = SimpleBinActive.apply
    ...
    def forward(self, x):
        out = self.bn1(x)
        #out = self.binary_activation1.apply(out)
        out = self.binary_activation1(out)
        #out = F.relu(self.binaryconv1(out))
        out = self.prelu1(self.binaryconv1(out))
        out = self.bn2(out)
        #out = self.binary_activation2.apply(out)
...
#{{{
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.bn2(F.relu(x))
        # for ImageNet, maxpooling is required
        #x = self.maxpool(x)
...
def birealnet18(pretrained=False, **kwargs):
    ...
    model = BiRealNet(BasicBlock_BiReal, [4, 4, 4, 4], **kwargs)
    return model


def birealnet34(pretrained=False, **kwargs):
    """Constructs a BiRealNet-34 model. """
    model = BiRealNet(BasicBlock_BiReal, [6, 8, 12, 6], **kwargs)
    return model
